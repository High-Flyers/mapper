...
def align_images_orb(img1, img2):
    ...
    if len(matches) < 10:
        print("Za mało dopasowań ORB, zwracam obraz bez zmian.")
...
def generate_map(folder_path="map_visualization/samples", img_width=1280, img_height=720, fov_x=1.74, ortho_width = 5000, ortho_height = 5000,
                 preview_scale=1, alpha=.5, show_preview=True):
    ...
    for info in images_info:
        ...
        if info != ref:
            try:
                img_np, H = align_images_orb(prev_img_np, img_np)
                print(f"✅ Zastosowano homografię ORB dla {info['path']}")
            except Exception as e:
                print(f"❌ ORB nie powiodło się dla {info['path']}: {e}")
        h, w = img_np.shape[:2]
